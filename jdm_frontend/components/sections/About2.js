...
export default function About2() {
  ...
  const apiV1Base = process.env.NEXT_PUBLIC_API_URL_V1 || `${apiBase}/api/v1`;

  useEffect(() => {
    const fetchAboutData = async () => {
      try {
        const response = await fetch(`${apiV1Base}/about/`, {
          cache: "no-store", // Fresh data every time
        });
        const data = await response.json();
        setAboutData(data); // v1 API returns object directly
      } catch (err) {
                  ...
                  {aboutData?.CEO_Photo && aboutData?.CEO_Name && (
                    <div className="about-author">
                      <div className="author-image wow fadeInUp" data-wow-delay=".2s">
                        <img
                          src={`${apiBase}${aboutData.CEO_Photo}`}
                          alt="ceo-img"
